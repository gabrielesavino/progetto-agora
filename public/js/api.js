/**
 * AGORÀ — api.js v5
 * Layer fetch: auth, materiali, admin IAM.
 */
...
async function login(username, password) {
  try {
    ...
    if (r.successo) { salvaSessione(r.utente); reindirizzaPerRuolo(r.utente.ruolo); return { successo: true, messaggio: r.messaggio }; }
    return { successo: false, messaggio: r.messaggio, bannato: r.codice === 'ACCOUNT_BANNED' };
  } catch { return { successo: false, messaggio: 'Impossibile contattare il server.' }; }
}

/**
 * Registrazione — ora accetta anche nationality.
 */
...
async function caricaMateriale(file, titolo, materia, username, categoriaCustom = '') {
  try {
    ...
    d._statusCode = r.status;
    return d;
  } catch { return { successo: false, messaggio: 'Errore di rete durante il caricamento.' }; }
}
...
async function adminAggiornaUtente(targetUsername, nuovoRuolo, nuovoStato) {
  ...
  } catch { return { successo: false, messaggio: 'Errore di rete.' }; }
}